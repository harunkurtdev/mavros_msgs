...
class OnboardComputerStatus extends RosMessage<OnboardComputerStatus> {
  ...
  void serialize(ByteDataWriter writer) {
    ...
    if (cpu_cores.length != 8) {
      throw Exception(
          'Unable to serialize array field cpu_cores - length must be 8');
    }
    // Serialize message field [cpu_cores]
    writer.writeArray<int>(cpu_cores, (val) => writer.writeUint8(val),
        specArrayLen: 8);
    // Check that the constant length array field [cpu_combined] has the right length
    if (cpu_combined.length != 10) {
      throw Exception(
          'Unable to serialize array field cpu_combined - length must be 10');
    }
    // Serialize message field [cpu_combined]
    writer.writeArray<int>(cpu_combined, (val) => writer.writeUint8(val),
        specArrayLen: 10);
    // Check that the constant length array field [gpu_cores] has the right length
    if (gpu_cores.length != 4) {
      throw Exception(
          'Unable to serialize array field gpu_cores - length must be 4');
    }
    // Serialize message field [gpu_cores]
    writer.writeArray<int>(gpu_cores, (val) => writer.writeUint8(val),
        specArrayLen: 4);
    // Check that the constant length array field [gpu_combined] has the right length
    if (gpu_combined.length != 10) {
      throw Exception(
          'Unable to serialize array field gpu_combined - length must be 10');
    }
    // Serialize message field [gpu_combined]
    writer.writeArray<int>(gpu_combined, (val) => writer.writeUint8(val),
        specArrayLen: 10);
    ...
    if (temperature_core.length != 8) {
      throw Exception(
          'Unable to serialize array field temperature_core - length must be 8');
    }
    // Serialize message field [temperature_core]
    writer.writeArray<int>(temperature_core, (val) => writer.writeint8(val),
        specArrayLen: 8);
    // Check that the constant length array field [fan_speed] has the right length
    if (fan_speed.length != 4) {
      throw Exception(
          'Unable to serialize array field fan_speed - length must be 4');
    }
    // Serialize message field [fan_speed]
    writer.writeArray<int>(fan_speed, (val) => writer.writeint16(val),
        specArrayLen: 4);
  ...
  OnboardComputerStatus deserialize(ByteDataReader reader) {
    ...
    data.cpu_cores =
        reader.readArray<int>(() => reader.readUint8(), arrayLen: 8);
    // Deserialize message field [cpu_combined]
    data.cpu_combined =
        reader.readArray<int>(() => reader.readUint8(), arrayLen: 10);
    // Deserialize message field [gpu_cores]
    data.gpu_cores =
        reader.readArray<int>(() => reader.readUint8(), arrayLen: 4);
    ...
    data.temperature_core =
        reader.readArray<int>(() => reader.readint8(), arrayLen: 8);
    // Deserialize message field [fan_speed]
    data.fan_speed =
        reader.readArray<int>(() => reader.readint16(), arrayLen: 4);
    ...
    data.storage_type =
        reader.readArray<int>(() => reader.readUint32(), arrayLen: 4);
    ...
    data.link_type =
        reader.readArray<int>(() => reader.readUint32(), arrayLen: 6);
